...
/**
 * Parse a version string into a semver_t structure
...
int semver_parse_string(const char *version_str, semver_t *ver) {
  ...
  return semver_parse(version_str, ver);
}
...
int semver_compare_strings(const char *version1, const char *version2) {
  if (!version1 || !version2) {
    return 0; // Consider both equal if either is null
  }
  
  semver_t v1, v2;
  
  if (semver_parse_string(version1, &v1) != 0) {
    return 0; // Parse error, consider equal
  }
  
  if (semver_parse_string(version2, &v2) != 0) {
    semver_free(&v1);
    return 0; // Parse error, consider equal
  }
...
/**
 * Check if currentVersion >= targetVersion
 * Returns: true if currentVersion >= targetVersion, false otherwise
 */
int semver_gte_string(const char *currentVersion, const char *targetVersion) {
  if (!currentVersion || !targetVersion) {
    return 0; // false if either version is null
  }
  
  semver_t current, target;
  
  if (semver_parse_string(currentVersion, &current) != 0) {
    return 0; // Parse error, return false
  }
  
  if (semver_parse_string(targetVersion, &target) != 0) {
    semver_free(&current);
    return 0; // Parse error, return false
  }
  
  int result = semver_gte(current, target);
